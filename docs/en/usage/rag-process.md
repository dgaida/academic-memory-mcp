...
To answer emails professionally and contextually, the system uses a multi-stage RAG process. This process ensures that the LLM has access to specific knowledge (such as examination regulations or module descriptions) stored in the vector databases of the respective email classes.
...
- These chunks are inserted as "Additional Context" into the prompt for the final response generation.  
