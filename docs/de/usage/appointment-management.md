...
*   **Studenten-Kontext:** Bei Auswahl eines Termins sucht das System automatisch nach dem passenden Studenten-Ordner.  
    *   **Wie findet er den Ordner?**  
        ...
        1. **Namensextraktion:** Der Nachname wird über reguläre Ausdrücke und Textmuster aus dem Teilnehmer-String gefiltered (z. B. "Mustermann" aus `max.mustermann@th-koeln.de` oder "Mustermann" aus `Mustermann Max <... >`).
...
Wenn das System eine E-Mail beantwortet (Aktion 1: "Antwort schreiben"), prüft es automatisch im Hintergrund auf allgemeine Terminanfragen, konkrete Terminvorschläge oder Terminbestätigungen (Zusagen) seitens des Absenders.

### Funktionsweise

1. **Erkennung:** Das LLM analysiert die eingehende Mail auf allgemeine Terminanfragen, konkrete Vorschläge (z. B. "Passt es am Dienstag um 14:00?") oder Zusagen (z. B. "Ich nehme den Termin am Montag um 15:30 Uhr").
...
3. **Alternativenvorschlag bei Konflikten / allgemeine Terminanfragen:**
    - Falls ein Konflikt erkannt wird oder eine allgemeine Terminanfrage ohne konkreten Termin vorliegt, liest das System automatisch die freien Terminslots aus `data/free_slots.md` (über das Tool `get_appointment_slots`) ein.
    - Es schlägt diese freien Termine als Alternativen in der Antwort-E-Mail vor und bittet den Absender um eine Auswahl.
