...
class Crawler:
    ...
    def crawl(self) -> None:
        ...
        for folder_path in self.config.folders.folders:
            ...
            if path.exists() and path.is_dir():
                ...
                coll_name = path.name
                exts = [
                    ext.lstrip(".") for ext in self.config.folders.supported_extensions
                ]
                mask = f"**/*.{{{','.join(exts)}}}"
                ...
                logger.warning(
                    f"Configured folder does not exist or is not a directory: {folder_path}"
                )
    ...
    def _process_directory(
        self, dir_path: Path, parent_id: Optional[int] = None
        ...
        if "Inbox" in subdirs and "SentItems" in subdirs:
            ...
            conv_summary, conv_changed = self._process_email_conversation(
                dir_path, folder_id
            )
            if conv_summary:
        ...
        for entry in os.scandir(dir_path):
            ...
            if entry.is_dir():
                ...
                sub_summary, sub_changed = self._process_directory(
                ...
                if entry.name.startswith(".") and (
                    ...
                    or entry.name.endswith(".emails_summary.md")
                ):
                    continue
        ...
        if item_summaries:
            if any_changed or not existing_folder_summary:
                ...
                folder_summary = self.summarizer.summarize_folder(
                    dir_path.name, item_summaries
                )
                if folder_summary:
                ...
                logger.info(
                    f"Folder {dir_path.name} unchanged. Skipping re-summarization."
                )
                folder_summary = existing_folder_summary

        return folder_summary, any_changed

    def _process_email_conversation(
        self, dir_path: Path, folder_id: int
    ) -> Tuple[Optional[str], bool]:
        ...
        db_folder = (
            ...
            .execute("SELECT identity_json FROM folders WHERE id=?", (folder_id,))
            .fetchone()
        )
        ...
        logger.info(
            f"Processing {len(email_files)} emails for conversation summary in {dir_path.name}"
        )
        ...
        summary = self.summarizer.summarize_email_conversation(
            dir_path.name, conversation_content
        )
        if summary:
            # Store hash in identity_json
            with self.store._get_connection() as conn:
                conn.execute(
                    "UPDATE folders SET identity_json = ? WHERE id = ?",
                    (combined_hash, folder_id),
                )
                conn.commit()
    ...
    def _process_file(
        self, file_path: Path, folder_id: int
    ) -> Tuple[Optional[str], bool]:
        ...
        file_id = self.store.upsert_file(
            str(file_path), file_hash, mtime, file_path.suffix.lower(), folder_id
        )
