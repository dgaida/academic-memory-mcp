...
def test_delayed_summary_logic(mock_agent, mock_summarizer, mock_profiler, mock_parser_cls, mock_config, tmp_path):
    ...
    controller.summarizer.summarize_email_conversation.return_value = "# Delayed Summary"

    with patch("mcp_university.classifier.controller.create_outlook_draft", return_value=True),          patch.object(controller, "generate_reply", return_value=("Subject", "Reply", False)):
    ...
    with patch("mcp_university.classifier.controller.create_outlook_draft", return_value=True),          patch.object(controller, "generate_reply", return_value=("Subject", "Reply", False)):
        controller.execute_action(0, mail_path, {"lastname": "Mustermann", "class": "Bachelor_Thesis", "identifier_path": student_dir})
