...
def test_execute_action_archive(mock_controller_deps, tmp_path):
    """Test function docstring."""
    controller, _ = mock_controller_deps
    res = controller.execute_action(1, tmp_path / "mail.msg", {})
...
def test_generate_reply_archive(mock_controller_deps, tmp_path):
    ...
    # 1 = archive
    res = controller.generate_reply(mail_path, "summary", Path("skill"), "doc", Path("persona"), "ctx", Path("apt"), "Sender", "student@test.de", action_idx=1)
