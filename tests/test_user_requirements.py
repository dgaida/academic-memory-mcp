"""Tests für nutzerspezifische Sortieranforderungen."""
import pytest
...
def test_requirement_1_name_extraction() -> None:
    """Anforderung 1: Spezifische Namensextraktionen."""
    test_cases = [
...
def test_requirement_2_cc_inbox(mock_deps: Any, tmp_path: Path) -> None:
    """Anforderung 2: E-Mail in CC sollte in Inbox landen.
    ...
    rec1 = create_recipient("other@smail.th-koeln.de", "Other Student", 1)
    rec2 = create_recipient("daniel.gaida@th-koeln.de", "Daniel Gaida", 2)
...
def test_requirement_2_cc_sent_self(mock_deps: Any, tmp_path: Path) -> None:
    """Anforderung 2: Nutzer sendet an sich selbst in CC -> SentItems.
    ...
    rec1 = create_recipient("student@smail.th-koeln.de", "Max Mustermann", 1)
    rec2 = create_recipient("daniel.gaida@th-koeln.de", "Daniel Gaida", 2)
    mock_msg.recipients = [rec1, rec2]
...
def test_requirement_3_inbox_cases(mock_deps: Any, tmp_path: Path) -> None:
    """Anforderung 3: Spezifische Inbox-Speicherung.
...
def test_requirement_4_sent_items_prioritization(mock_deps: Any, tmp_path: Path) -> None:
    """Anforderung 4: Bevorzugung von TO-Empfängern.
    ...
    rec1 = create_recipient("eva.adam@hans-gmbh.com", "Eva Adam | Hans GmbH", 1)
    rec2 = create_recipient("hans.dampf@th-koeln.de", "hans.dampf@th-koeln.de", 2)
    mock_msg.recipients = [rec1, rec2]
