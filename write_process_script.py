...
def run_sort_emails(source_dir: str, config_path: str, method: str = "transformer", mode: str = "combined") -> None:
    ...
    if not model_path.exists():
        logger.error(f"Modell-Datei nicht gefunden: {model_path}. Programm wird beendet.")
        sys.exit(1)
...
def main() -> None:
    ...
    parser.add_argument("--method", default="transformer", help="Klassifizierungsmethode (transformer, xgboost, randomforest)")
    parser.add_argument("--mode", default="combined", help="Merkmalsextraktion (tfidf, embedding, combined)")
    args = parser.parse_args()

    source_dir = Path(args.source_dir)
    # 1. Sortieren
    try:
        run_sort_emails(str(source_dir), args.config, method=args.method, mode=args.mode)
    except Exception as e:
